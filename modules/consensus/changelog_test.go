...
func TestIntegrationChangeLog(t *testing.T) {
	...
	cst.cs.ConsensusSetSubscribe(cst.cs.tg.StopChan(), &ms, modules.ConsensusChangeBeginning)
	...
	cst.cs.ConsensusSetSubscribe(cst.cs.tg.StopChan(), &tailSubscriber, tailSubscriber.updates[len(tailSubscriber.updates)-1].ID)
	...
	cst.addSiafunds()
	cst.mineSiacoins()
	cst.cs.ConsensusSetSubscribe(cst.cs.tg.StopChan(), &behindSubscriber, behindSubscriber.updates[len(behindSubscriber.updates)-1].ID)
